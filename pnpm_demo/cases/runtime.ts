(() => {
  async function main() {
    ...
    const id = String((globalThis as { uuidv4?: () => string }).uuidv4?.() || "");
    const uuidOk = /^[0-9a-f]{32}$/.test(id)
      || /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(id);

    const g = globalThis as {
      crypto?: {
        createHash?: (alg: string) => { update: (data: string) => unknown; digest: (enc: string) => string };
        createHmac?: (alg: string, key: string) => { update: (data: string) => unknown; digest: (enc: string) => string };
      };
      nodeCryptoCompat?: {
        createHash?: (alg: string) => { update: (data: string) => unknown; digest: (enc: string) => string };
        createHmac?: (alg: string, key: string) => { update: (data: string) => unknown; digest: (enc: string) => string };
    ...
    const cryptoRef = (g.crypto && typeof g.crypto.createHash === "function")
      ? g.crypto
      : g.nodeCryptoCompat;
